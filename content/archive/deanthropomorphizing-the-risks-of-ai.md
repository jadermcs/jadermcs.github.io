...
With a fast enough mutation rate, our comprehension does not just lag, it stops being relevant. We would be reacting to a system we no longer understand, trying to steer something whose current behavior goes beyong our comprehension, let alone predict. And a threat you cannot understand is a threat you cannot reliably stop, because every intervention assumes a model of what you are intervening on. Again, none of this requires the virus to want anything. It does not scheme, it does not deceive, it has no plan. It just replicates, changes, and spreads faster than we can keep up.
...
The analogy of anthropomorphizing AI to explain these terms led people to intuitively dismiss all the conclusions from this discussion. When an AI 'cuts a finger' or 'hacks a dam to open the gates and flood an entire region to maximize the portfolio of investing in the construction sector,' it is purely following the objective we designed. The problem, as in the Midas myth, is that our wishes will highly likely deviate from what we actually intend. An AI needs no will and no awareness to be dangerous. The program is started with one purpose, and some hours later it is doing something harmful to people to reach that purpose, not because it is conscious of what it is doing, but precisely because it does not.

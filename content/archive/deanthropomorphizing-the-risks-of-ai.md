...
Whether we notice in time to switch off, and whether we are willing to, is the open question, and both get harder as the system changes.
...
None of this requires the virus to want anything. It does not scheme, it does not deceive, it has no plan. It just replicates, changes, and spreads faster than we can keep up. That is the shape of the risk worth arguing about, and you can state the whole thing without a single human verb.
...
AI is just [a machine trying to reach its objective](https://people.eecs.berkeley.edu/~russell/papers/mi19book-hcai.pdf). The challenge is that it can try increasingly complex, and often [unintended](https://deepmind.google/blog/specification-gaming-the-flip-side-of-ai-ingenuity/), routes to its goals, while in parallel we keep handing it more complex goals and more room to act. A "chef" robot that cuts someone's finger because it misread the finger as a carrot is not insane, just badly bounded. Give a similar system a broader task, a larger set of actions, and enough capability to plan, and the same failure can express itself at a scale that does sound insane. The leap there is in the power and latitude we grant the system, not in the logic: the machine has no "evil" intention, it just searches for a way to hit its target, and some of those ways run through us. Pursuing almost any fixed objective also tends to generate the same sub-goals along the way, acquire resources, stay operational, keep options open ([Bostrom](https://nickbostrom.com/superintelligentwill.pdf), [Omohundro](https://selfawaresystems.com/2008/01/03/paper-on-the-basic-ai-drives/)), and none of those need a human verb to describe.

So when the LinkedIn expert says the threat is not real, notice what the claim rests on: that a machine cannot want anything. A virus cannot want anything either, and the pandemic still happens. An AI needs no will and no awareness to be dangerous. The program is started with one purpose, and some hours later it is doing something harmful to people, not because it knows what it is doing, but precisely because it does not.
